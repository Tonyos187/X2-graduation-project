...
const SearchFild: React.FC<Props> = ({ data }) => {
  ...
  const renderButton = (btn: CTAButton) => (
    <button
      className="flex items-center justify-center gap-2 w-fit lg:w-46 h-[63px] px-[24px] py-[18px] rounded-[10px] bg-mainPurple text-white hover:bg-secPurple transition"
    >
      {btn.icon && (
        <img src={btn.icon.toString()} alt="search icon" className="w-4 h-4" />
      )}
