...
import PropertyMap from "../kit/PropertyMap";
...
function PropertiesCard({
  ...
  return (
    <div
      className="w-full h-full bg-Grey-08 border border-Grey-15 p-6 md:p-7.5 xl:p-10 rounded-xl grid transition-transform duration-700 ease-in-out"
      style={{
        perspective: "800px",
    ...
    >
      {/* FRONT FACE */}
      <div
        className="flex flex-col justify-between gap-7.5 h-full"
        style={{
          gridArea: "front",
          gridRow: "1 / -1",
          gridColumn: "1 / -1",
      ...
      >
        <img
          src={image}
          alt=""
          className="w-full h-[210px] md:h-[255px] xl:h-[318px] object-cover rounded-[10px]"
        />
        {!showInfo && (
          <div className="py-1.5 px-3 xl:py-2 xl:px-3 bg-Grey-10 rounded-[28px] border border-Grey-15 max-w-max">
            <p className="text-sm xl:text-lg text-White max-w-max">
              {discoverDescription}
            </p>
          </div>
        )}
        ...
        <div className="flex justify-between flex-wrap gap-y-4">
          <div>
            <span className="text-sm xl:text-lg font-medium text-Grey-60">
              Price
            </span>
            <p className="font-semibold text-lg md:text-xl xl:text-2xl">
              {price}
            </p>
          </div>
      ...
      >
        ...
        {position && (
          <PropertyMap
            position={position}
            title={title}
            className="w-full h-40 md:h-56 rounded-[10px] overflow-hidden"
          />
        )}
