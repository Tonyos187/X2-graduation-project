...
const SharedForm = ({data, areaText, detailsForm} : {data: ConactFormType[] , areaText: ConactFormType , detailsForm?: boolean}) => {
  ...
  return (
    <form className={`rounded-xl p-5 md:p-10 border border-Grey-15 flex flex-col gap-5 ${detailsForm ? 'lg:p-10 xl:p-12.5 md:gap-5 xl:gap-7.5' : 'lg:p-20 xl:p-25 md:gap-7.5 xl:gap-12.5 '}`}>
      ...
      <div className={`flex flex-col lg:flex-row justify-between gap-5 w-full mt-2.5 ${detailsForm ? 'md:mt-5' : 'md:mt-0'}`}>
        <div className="flex gap-1.5 xl:gap-2.5 items-center flex-1">
            <div className="relative w-6 h-6 xl:w-7 xl:h-7">
                ...
                {checked === false && <span onClick={() => setChecked(!checked)} className="absolute inset-0 top-0 left-0 w-6 h-6 xl:w-7 xl:h-7 border border-Grey-15 bg-Grey-10 rounded-sm cursor-pointer"></span>}
            </div>
